...
class TextDataset(BaseDataset):
    ...
    def __init__(self, file_paths: List[str], block_size: int, tokenizer: Tokenizer):
        """
        ...
        if hasattr(self.tokenizer, 'load') and hasattr(self.tokenizer, 'config') and isinstance(self.tokenizer.config, dict) and 'model_path' in self.tokenizer.config:
            ...
            if model_path_rel:
                 ...
                 try:
                     self.logger.info(f"Attempting to load tokenizer model from: {model_path_abs}")
                     self.tokenizer.load(model_path_abs) # Assuming load method takes path
                     self.logger.info(f"Tokenizer model loaded successfully.")
                 except Exception as e:
                     self.logger.error(f"Failed to load tokenizer model from {model_path_abs}: {e}", exc_info=True)
                 ...
                 self.logger.warning("Tokenizer has 'load' method and 'model_path' in config, but path is empty. Proceeding without explicit load.")
        elif hasattr(self.tokenizer, 'load'):
            # Log if load method exists but no path configured
            self.logger.info("Tokenizer has a 'load' method, but no model path was configured. Assuming pre-initialized or no loading needed.")
        # -------------------------------------------------- #
        
        # --- Read and Tokenize Text Data --- #
        all_text = ""
        self.logger.info(f"Loading and concatenating text data from: {self.file_paths}")
        for file_path in self.file_paths:
            try:
                 # Resolve path using Hydra utils if it might be relative
                 abs_file_path = hydra.utils.to_absolute_path(file_path)
                 if not os.path.exists(abs_file_path):
                      self.logger.error(f"Text file not found: {abs_file_path} (resolved from {file_path})")
                      continue # Skip missing files
                      
                 with open(abs_file_path, 'r', encoding='utf-8') as f:
                     all_text += f.read()
                 self.logger.debug(f"Read content from {abs_file_path}")
            except Exception as e:
                self.logger.error(f"Failed to read file {file_path}: {e}", exc_info=True)
                # Decide if this should be a fatal error
        
        if not all_text:
            self.logger.error("No text data could be loaded from provided file paths.")
        ...
        self.logger.info(f"Loaded total {len(all_text)} characters. Tokenizing...")
        try:
            # Encode the entire concatenated text
            self.token_ids = self.tokenizer.encode(all_text)
            # Convert to tensor
            self.token_ids = torch.tensor(self.token_ids, dtype=torch.long)
            self.vocab_size = self.tokenizer.get_vocab_size()
            self.logger.info(f"Tokenization complete. Total tokens: {self.token_ids.numel()}, Vocab size: {self.vocab_size}")
            ...
            self.logger.error(f"Failed during tokenization: {e}", exc_info=True)
    ...
    def decode(self, tokens: Union[torch.Tensor, List[int]], skip_special_tokens: bool = False) -> str:
        ...
        if not self.tokenizer:
            self.logger.warning("Cannot decode: Tokenizer not available.")
            return "[Decode unavailable: No tokenizer]"
        try:
            return self.tokenizer.decode(tokens, skip_special_tokens=skip_special_tokens)
        except Exception as e:
            self.logger.error(f"Error during tokenizer decoding: {e}", exc_info=True)
