...
class TrainingConfig:
    ...
    use_torch_compile: bool = False  # Alias for torch_compile for backward compatibility
    compile_mode: str = "default"  # Mode for torch.compile
    
    # Checkpointing
    save_interval: int = 0  # Save every N steps (0 to disable)
    time_save_interval_minutes: int = 0  # Save every N minutes (0 to disable)
    checkpoint_subdir: str = "checkpoints"  # Subdirectory for checkpoints
    ...
    def __post_init__(self):
        ...
        # Ensure both torch_compile flags are in sync
        self.use_torch_compile = self.torch_compile
        
        # Validate compile mode
        valid_modes = ["default", "reduce-overhead", "max-autotune"]
        if self.compile_mode not in valid_modes:
            raise ValueError(f"Invalid compile_mode '{self.compile_mode}'. Must be one of {valid_modes}")
        
        # Validate intervals
        if self.save_interval < 0:
            raise ValueError(f"save_interval must be >= 0, got {self.save_interval}")
        if self.time_save_interval_minutes < 0:
            raise ValueError(f"time_save_interval_minutes must be >= 0, got {self.time_save_interval_minutes}")
