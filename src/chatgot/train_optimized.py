...
This script provides an optimized version of the training process that incorporates
all the performance enhancements for maximum throughput, including:

1. Optimal batch size selection
2. CUDA-specific optimizations
3. Gradient accumulation with optimal step size
4. Efficient memory usage strategies
5. Enhanced progress monitoring

Usage:
    python -m src.train_optimized --data_path processed_data/got_char_data.pkl
"""
...
def optimize_training_parameters(args, device, is_cuda, char_to_idx) -> argparse.Namespace:
    """
    ...
    """
    log_section_header("OPTIMIZING TRAINING PARAMETERS")
    ...
    if args.optimize_batch_size and is_cuda:
        from src.batch_size_finder import find_optimal_batch_size as bsf
...
def main() -> None:
    ...
    # Optimize training parameters if requested
    if args.optimize_cuda or args.optimize_batch_size:
    ...
    if args.use_8bit_optimizer or args.use_8bit_adam or args.use_8bit_adamw:
        try:
            import bitsandbytes as bnb
            logging.info("Using 8-bit optimizers from bitsandbytes")
            ...
            if args.use_8bit_adamw:
                ...
                logging.info("Creating 8-bit Adam optimizer")
            ...
            else:  # default to 8-bit optimizer
                logging.info("Creating 8-bit optimizer")
                optimizer = bnb.optim.SGD8bit(
                    model.parameters(),
                    lr=args.lr
                )
            
            logging.info("8-bit optimizer successfully created")
        except ImportError:
    ...
    if args.use_8bit_optimizer or args.use_8bit_adam or args.use_8bit_adamw:
        if args.use_8bit_adamw:
            logging.info(f"Optimizer: 8-bit AdamW")
        elif args.use_8bit_adam:
            logging.info(f"Optimizer: 8-bit Adam")
        else:
            logging.info(f"Optimizer: 8-bit SGD")
    else:
